...
.
  ...
  plugins/fluxo-trabalho/           # plugin de fluxo de trabalho
    ...
    index.ts                       # plugin OpenCode (chat.message + session.idle; registra agente commit-agent)
    package.json                   # pacote npm @nomadiction8991/fluxo-trabalho
    agents/commit-agent.md         # subagente de commit (Claude); OpenCode registra no index.ts
...
- Subagentes do Claude Code ficam em `plugins/fluxo-trabalho/agents/*.md` (auto-discovery). No OpenCode o equivalente é registrado no hook `config` do `index.ts` (`cfg.agent`). Ao criar/atualizar agente, mantenha os dois em sincronia (mesmo nome, mesma descrição de gatilho, mesmo prompt).
