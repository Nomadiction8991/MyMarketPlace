...
### Installed via the 9router plugin (Claude Code / OpenCode marketplace)

When the 9router plugin is active, defaults are already provided — no manual setup needed:

- `NINEROUTER_URL` defaults to `https://9router.anvy.com.br` (override via env if you run your own instance).
- `NINEROUTER_KEY` is injected at runtime from the plugin's token (env `NINEROUTER_KEY` > `CLAUDE_LIMIT_PROXY_AUTH_TOKEN` > secrets file with mode 600).

If both variables are unset, use `NINEROUTER_URL=https://9router.anvy.com.br` and the plugin token directly.
